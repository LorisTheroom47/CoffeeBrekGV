-- Coffee Break GV
-- Punti di consegna ospedaliera estesi e orari delivery a slot tra le 12:00 e le 14:00.
-- La consegna resta gratuita e l'idempotenza della RPC resta invariata.
-- Migrazione predisposta per revisione e applicazione manuale.

begin;

-- Il vincolo resta NOT VALID per non rivalidare retroattivamente gli ordini
-- storici. PostgreSQL lo applica comunque a ogni nuova riga o modifica.
alter table public.orders
  drop constraint if exists orders_fulfillment_details_valid;

alter table public.orders
  add constraint orders_fulfillment_details_valid
  check (
    (
      ...
      and (
        ...
        or
        (
          delivery_point is null
          and delivery_address is not null
          and pg_catalog.btrim(delivery_address) <> ''
          and delivery_city is not null
          and pg_catalog.btrim(delivery_city) <> ''
          and delivery_postal_code is not null
          and pg_catalog.btrim(delivery_postal_code) <> ''
        )
      )
    )
    or
...
alter table public.orders
  drop constraint if exists orders_delivery_time_valid;

alter table public.orders
  add constraint orders_delivery_time_valid
  check (
    fulfillment_type <> 'delivery'
    or (
      requested_time is not null
      and requested_time in (
        time '12:00:00',
        time '12:15:00',
        time '12:30:00',
        time '12:45:00',
...
is 'Crea atomicamente un ordine idempotente con ritiro o consegna ospedaliera gratuita nei punti A, B, C, Pronto Soccorso o Palazzina Blu; per la consegna accetta soltanto slot di 15 minuti tra le 12:00 e le 14:00, usando prezzi e disponibilità correnti.';
