-- Coffee Break GV
-- Consegna ospedaliera nei punti A/B/C e firma RPC aggiornata.
-- Migrazione predisposta per revisione e applicazione manuale.
-- Gli ordini storici restano invariati; il nuovo vincolo NOT VALID è applicato
-- a tutti i nuovi inserimenti senza richiedere la riscrittura dei dati esistenti.

begin;

alter table public.orders
  add column if not exists delivery_point pg_catalog.text null;
...
-- Rimuove la firma idempotente precedente, che richiedeva indirizzo/città/CAP.
...
is 'Crea atomicamente un ordine idempotente con ritiro o consegna ospedaliera nei punti A, B o C, usando prezzi e disponibilità correnti.';
