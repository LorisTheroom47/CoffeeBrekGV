-- Coffee Break Monza
-- Idempotenza atomica per la creazione degli ordini pubblici.
-- Migrazione predisposta per revisione e applicazione manuale.
-- Il fingerprint SHA-256 è calcolato dalla Server Action Next.js dopo la
-- normalizzazione e non proviene dal browser. PostgreSQL non calcola l'hash:
-- la RPC, eseguibile soltanto da service_role, ne valida formato e coerenza.

begin;

alter table public.orders
  add column if not exists idempotency_key pg_catalog.uuid null;

alter table public.orders
  add column if not exists request_fingerprint pg_catalog.text null;
...
begin
  if not exists (
    ...
    where
      ...
      and constraint_definition.conname = 'orders_idempotency_key_key'
  ) then
    alter table public.orders
  ...
  if not exists (
    ...
    where
      constraint_definition.conrelid = 'public.orders'::pg_catalog.regclass
      and constraint_definition.conname = 'orders_idempotency_fields_together'
  ) then
    alter table public.orders
      add constraint orders_idempotency_fields_together
      check (
  ...
  if not exists (
    ...
    where
      constraint_definition.conrelid = 'public.orders'::pg_catalog.regclass
      and constraint_definition.conname = 'orders_request_fingerprint_valid'
  ) then
    alter table public.orders
      add constraint orders_request_fingerprint_valid
      check (
...
-- La vecchia firma non deve restare invocabile come overload alternativo.
...
-- Consente di rieseguire ragionevolmente la migrazione durante la revisione.
drop function if exists public.create_public_order(
  pg_catalog.text,
  pg_catalog.text,
  pg_catalog.text,
...
is 'Crea atomicamente un ordine idempotente usando prezzi e disponibilità correnti; i retry restituiscono lo stesso ordine senza nuovi inserimenti.';
