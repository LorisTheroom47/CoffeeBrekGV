-- Coffee Break GV
-- Separazione tra visibilità nel menu e ordinabilità online dei piatti.
-- La RPC accetta soltanto piatti disponibili e ordinabili.
-- Migrazione predisposta per revisione e applicazione manuale.

begin;

alter table public.menu_items
  add column if not exists orderable pg_catalog.bool default true;

update public.menu_items
set orderable = true
where orderable is null;

alter table public.menu_items
  alter column orderable set default true,
  alter column orderable set not null;
...
where
  ...
  and pg_catalog.lower(pg_catalog.btrim(category.name)) = 'caffetteria';

comment on column public.menu_items.orderable
is 'Indica se il piatto, pur restando visibile nel menu, può essere ordinato online.';
...
is 'Crea atomicamente un ordine idempotente usando esclusivamente piatti disponibili e ordinabili, con ritiro o consegna ospedaliera gratuita nei punti e negli orari consentiti.';
